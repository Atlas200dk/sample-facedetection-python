...
function check_board_ip()
{
    ...
    board_ip=$(cat ${app_path}/face_detection.conf | grep "atlas200dk_board_ip" | awk -F'[ =]+' '{print $2}')
	board_ip=$(echo $board_ip | sed -e 's/\r//' | sed -e 's/\n//' | sed -e 's/ //')
    if [[ "$board_ip" = "" ]];then
        echo "please check your face_detection.conf to make sure that each parameter has a value"
        return 1
    fi
